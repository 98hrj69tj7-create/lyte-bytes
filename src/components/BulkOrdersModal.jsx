...
export default function BulkOrdersModal({ isOpen, onClose }) {
  ...
  const submitBulkWhatsApp = (e) => {
    ...
    const newErrors = [];
    if (!bulkData.name.trim()) newErrors.push('name');
    ...
    const msg = `*Bespoke Catering & Bulk Inquiry*%0A- Name: ${bulkData.name}%0A- Phone: ${bulkData.phone}%0A- Event: ${finalEvent}%0A- Meal Slot: ${bulkData.mealTime}%0A- Guest Count: ${bulkData.guests}%0A- Event Date: ${bulkData.date}%0A- Interested Items: ${itemsList}%0A- Notes: ${bulkData.notes}`;
