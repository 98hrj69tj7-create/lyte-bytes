  ...
  const handleSend = (textToSend) => {
    ...
    setTimeout(() => {
      ...
      if (matchedFaq) {
        ...
        // Fallback response with WhatsApp redirect prompt
        setMessages(prev => [
          ...prev, 
          { 
            sender: 'bot', 
      ...
      <div style={{ 
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '10px', overflowY: 'auto', maxHeight: '340px', paddingBottom: '10px' }}>
          {messages.map((msg, idx) => (
            <div key={idx} style={{ display: 'flex', justifyContent: msg.sender === 'user' ? 'flex-end' : 'flex-start', alignItems: 'flex-start', gap: '8px' }}>
              {/* Bot Avatar */}
              {msg.sender === 'bot' && (
                <div style={{ width: '26px', height: '26px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Bot size={30} color="#8A6D2B" />
                </div>
              )}

              {/* Message Bubble Container */}
              <div style={{ maxWidth: '82%', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ padding: '10px 14px', borderRadius: '14px', fontSize: '12px', lineHeight: '1.5', backgroundColor: msg.sender === 'user' ? '#FF5958' : '#FFFFFF', color: msg.sender === 'user' ? '#FFFFFF' : activeTheme.text, border: msg.sender === 'bot' ? '1px solid rgba(197, 160, 89, 0.3)' : 'none', boxShadow: '0 2px 8px rgba(0,0,0,0.03)', textAlign: 'left' }}>
                  {msg.text}
                </div>
        ...
        <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
          <input 
            ...
            style={{ flex: 1, padding: '10px 14px', borderRadius: '12px', border: '1px solid rgba(197, 160, 89, 0.4)', backgroundColor: '#FFFFFF', fontSize: '13px', outline: 'none', color: activeTheme.text }}
          />
          <button 
            type="button" 
            onClick={() => handleSend()} 
            style={{ background: 'linear-gradient(135deg, #FF5958 0%, #E11D48 100%)', color: '#FFFFFF', border: 'none', borderRadius: '12px', width: '42px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 12px rgba(255, 89, 88, 0.3)' }}
          >
            <Send size={16} />
          </button>
        </div>
