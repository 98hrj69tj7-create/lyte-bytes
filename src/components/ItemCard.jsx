...
export default function ItemCard({ item, addToCart, setSelectedItem, layout, resolveImagePath }) {
    ...
    <div style={{ 
      ...
      <div onClick={() => setSelectedItem(item)} style={{ cursor: 'pointer', flexShrink: 0 }}>
        <img 
          ...
          alt={item.name} 
          style={{ width: layout === 'grid' ? '100%' : '90px', height: layout === 'grid' ? '120px' : '90px', objectFit: 'cover', borderRadius: '12px' }} 
        />
      </div>
  ...
  <button 
    onClick={() => {
      // If the item has variants, open the modal to select; otherwise, add directly
